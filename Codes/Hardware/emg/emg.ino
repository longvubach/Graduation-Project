...
// ============================================================================
// SERVO CONTROL WITH HYSTERESIS + DEBOUNCING + CONFIRMATION
// ============================================================================
void controlServo() {
    // CRITICAL: Validate thresholds - ensure they are reasonable
    // thresholdLow MUST be > 0 to properly detect relaxed state
    if (thresholdLow <= 0 || thresholdLow < 2) {
        // Use at least 2, or 25% of high threshold, whichever is higher
        int minLow = max(2, thresholdHigh / 4);
        thresholdLow = minLow;
        Serial.println("  ⚠ WARNING: thresholdLow was <= 0 or too low, adjusted to " + String(thresholdLow));
        saveThresholds();
    }
    // Ensure minimum separation between thresholds (at least 30% of low threshold)
    ...
    if (thresholdHigh <= thresholdLow + minSeparation) {
        ...
        Serial.println("  ⚠ WARNING: thresholdHigh too close to thresholdLow, adjusted to " + String(thresholdHigh));
        saveThresholds();
    }
    ...
    // Detect current signal state (using validated thresholds)
    bool currentGripSignal = (emg1_filtered > thresholdHigh || emg2_filtered > thresholdHigh);
    bool currentRelaxSignal = (emg1_filtered < thresholdLow && emg2_filtered < thresholdLow);
    
    // Confirmation mechanism: require consistent signal for CONFIRMATION_DURATION
    if (currentGripSignal && !confirmedGripState) {
        if (gripConfirmationStart == 0) {
            ...
            relaxConfirmationStart = 0; // Reset relax confirmation
        }
    } else if (!currentGripSignal) {
        gripConfirmationStart = 0; // Reset if signal drops
    }
    ...
    if (currentRelaxSignal && confirmedGripState) {
        if (relaxConfirmationStart == 0) {
            ...
            gripConfirmationStart = 0; // Reset grip confirmation
        }
    } else if (!currentRelaxSignal) {
        relaxConfirmationStart = 0; // Reset if signal rises
    }
    
    // Rotate to 180° if grip is CONFIRMED and conditions are met
    if (confirmedGripState && currentAngle == 0 && !isRotating && canChangeState) {
        ...
        Serial.print("  EMG1: "); Serial.print(emg1_filtered);
        Serial.print(" | EMG2: "); Serial.print(emg2_filtered);
    ...
    // Return to 0° if relax is CONFIRMED and conditions are met
...
void handleTraining() {
    ...
    if (elapsed >= training.DURATION_MS) {
        ...
        // Start K-Means training automatically on device
        int durationSeconds = training.DURATION_MS / 1000;
        String startedMsg = "{\"status\":\"training_started\",\"duration\":" + String(durationSeconds) + "}";
        mqttClient.publish(topic_train, startedMsg.c_str());
        Serial.println("  → Running K-Means clustering now...");
        computeThresholdsKMeans();
        Serial.println("  ✓ K-Means training finished and thresholds published\n");
    }
}
